...
const UserRegistrationPage: React.FC = () => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch('http://localhost:3000/Admin/MenuPaciente');
    ...
    const handleRegister = async (e: React.FormEvent) => {
      ...
      if (NumeroCC === undefined || !Nombre || !Apellido || !email || !password || !EstadoUsuario || !rol || !tipoUsuario) {
    ...
    const saveChanges = async () => {
      if (editingUserId !== null) {
        ...
        try {
          const response = await fetch(`http://localhost:3000/Admin/MenuPaciente/${editingUserId}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
            },
    ...
    const handleDelete = async (NumeroCC: number) => {
      ...
      if (confirmDelete) {
        try {
          const response = await fetch(`http://localhost:3000/Admin/MenuPaciente/${NumeroCC}`, {
            method: "DELETE",
          });
